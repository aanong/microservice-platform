    ...
    public void onMessage(String message) {
        ...
        for (CacheSyncEvent event : events) {
            ...
            if (!markIdempotent(event)) {
                log.debug("Skip duplicated cache sync event. service=stock, eventId={}", resolveEventId(event));
                continue;
            }
            try {
                checkLag(event);
                ...
                log.info("Cache sync success. service=stock, eventId={}, table={}, opType={}, pk={}, sourcePos={}",
                    resolveEventId(event), event.getTable(), event.getOpType(), event.getPk(), event.getSourcePos());
            } catch (Exception ex) {
                log.error("Cache sync failed. service=stock, eventId={}, table={}, opType={}, pk={}, sourcePos={}",
