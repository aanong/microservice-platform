...
public class OrderCacheSyncConsumer implements RocketMQListener<String> {

    private static final Logger log = LoggerFactory.getLogger(OrderCacheSyncConsumer.class);
    ...
    public void onMessage(String message) {
        ...
        for (CacheSyncEvent event : events) {
            ...
            if (!markIdempotent(event)) {
                log.debug("Skip duplicated cache sync event. service=order, eventId={}", resolveEventId(event));
                continue;
            }
            try {
                checkLag(event);
                ...
                log.info("Cache sync success. service=order, eventId={}, table={}, opType={}, pk={}, sourcePos={}",
                    resolveEventId(event), event.getTable(), event.getOpType(), event.getPk(), event.getSourcePos());
            } catch (Exception ex) {
                log.error("Cache sync failed. service=order, eventId={}, table={}, opType={}, pk={}, sourcePos={}",
    ...
    private boolean markIdempotent(CacheSyncEvent event) {
        String key = "mall:cache:sync:dedup:order:" + buildEventKey(event);
